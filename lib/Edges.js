      ...
      },
      {
        ...
        value: function makeVerticesDraggable(vertices) {
          ...
          vertices.map(function(vertex) {
            _this3.plumbInstance.draggable(
              vertex.id,
              _objectSpread({}, _this3.props.draggableOptions, {
                stop: _this3.handleStop
              })
            );

            if (
              !_this3.plumbInstance
                .getElement(vertex.id)
                .classList.contains("jtk-droppable")
            ) {
              _this3.plumbInstance.droppable(
                vertex.id,
                _objectSpread({}, _this3.props.droppableOptions, {
                  drop: _this3.handleDrop
                })
              );
            }
          });
